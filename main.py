...
TK_WINDOW_TITLE = "Take a F****** Break!"  # Set title of Tk Window
...
SCREEN_WIDTH = "450"  # pixels wide
SCREEN_HEIGHT = "900"  # pixels high
SCREEN_X = "1910"  # pixel buffer from right side of screen
SCREEN_Y = "50"  # pixel buffer from top of screen
SCREEN_GEOMETRY = f"{SCREEN_WIDTH}x{SCREEN_HEIGHT}+{SCREEN_X}+{SCREEN_Y}"  # Placed in geometry method
...
def reset():
	global BREAK_INTERVALS  # Bring Constant in to change it back to zero
...
icon = tk.PhotoImage(file="coffee.png")  # Import an icon image and create photo image object
window.iconphoto(False, icon)  # Override default image with custom image
...
# window.attributes("-topmost", 1)  # Move the window to the top

# Create image at top of screen
